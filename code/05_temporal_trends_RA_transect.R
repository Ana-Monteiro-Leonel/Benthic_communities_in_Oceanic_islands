################################################################################
# Script: 05_temporal_trends_RA_transect.R
# Author: Monteiro-Leonel, Ana C.
# Date: 2026-04-01
# Description: 
#   Temporal trends analysis for Rocas Atoll (RA)
#   Identified as "rocas" in the dataset.
#   Groups raw categories into functional groups before analysis.
#   Manual selection of groups for visualization (prioritizing biological groups).
# Outputs:
#   - results/figures/Figure_5_temporal_trends_RA.png
#   - results/figures/Figure_5_temporal_trends_RA.tiff
#   - results/tables/Table_RA_summary_statistics.csv
...
df_bio_ra <- read.csv("data/raw/benthic_complete_data.csv") %>%
  filter(island == "rocas") %>%
  mutate(
...
# Check
sampling_effort <- df_bio_ra %>%
  group_by(group, year) %>%
  summarise(n_transects = n(), .groups = "drop")

write.csv(sampling_effort, "results/tables/Table_RA_sampling_effort.csv", 
          row.names = FALSE)

# 4. Summarize median cover by group ####
median_cover <- df_bio_ra %>%
  group_by(group) %>%
  summarise(
...
write.csv(median_cover, "results/tables/Table_RA_median_cover.csv")

cat("\n=== Median cover per group ===\n")
print(median_cover)

# MANUAL SELECTION OF GROUPS FOR TREND ANALYSIS ####
# For Rocas, we exclude ABI (sand/abiotic) and select the next biological groups
# Based on median cover, top groups are:
#   1. ABI (sand) - excluded (abiotic, not meaningful for ecological trends)
#   2. EAM - include
#   3. MAL - include  
#   4. CCA - include
#   5. ZOA - include (next biological group)

# Manually select groups for trend analysis (prioritizing biological groups)
...
cat("(Prioritizing biological groups, excluding ABI - sand)\n")
print(available_groups)

# Optional: Show why ABI was excluded
abi_median <- median_cover %>% filter(group == "ABI") %>% pull(median_cover)
if(length(abi_median) > 0) {
  cat("\nNOTE: ABI (sand) had median cover =", round(abi_median, 1), 
      "% but was excluded as it represents abiotic substrate.\n")
  zoa_median <- median_cover %>% filter(group == "ZOA") %>% pull(median_cover)
  if(length(zoa_median) > 0) {
    cat("      ZOA (median =", round(zoa_median, 1),
        "%) selected instead for ecological interpretation.\n")
  }  
}  
...
for(g in available_groups) {
  cat("\nAnalyzing group:", g, "\n")
  result <- analyze_trend(df_bio_ra, g)
  if(!is.null(result)) {
    results[[g]] <- result
...
if(length(results) > 0) {
  ...
  # Save outputs
  ggsave("results/figures/Figure_5_temporal_trends_RA.png", combined_plot, 
         width = 10, height = 8, dpi = 300)
  ggsave("results/figures/Figure_5_temporal_trends_RA.tiff", combined_plot, 
...
write.csv(stats_table, "results/tables/Table_RA_summary_statistics.csv", row.names = FALSE)
...
write.csv(trend_summary, "results/tables/Table_RA_trend_summary.csv", row.names = FALSE)

# 9. Print summary ####
cat("\n=== TREND ANALYSIS SUMMARY ===\n")
cat("Location: Rocas Atoll (rocas)\n")
